...
class FrontTemplateByTag extends Component
{
    public $tag_slug;
    public $search = null;
    ...
    {
        $templates = Template::withAnyTagsOfAnyType($this->tag_slug)->where('status', 1)->where('title', 'like', '%'.$this->search.'%')->get();
        $latests = Template::where('status', 1)->latest()->take(10)->get();
        $tags = Tag::get(['name', 'slug']);

        return view('livewire.front-template-by-tag', [
            'templates' => $templates,
            'tags' => $tags,
            'latests' => $latests,
            'tagslug' => $this->tag_slug,
        ]);
    }
}
