...
class FrontTemplateBySubCategory extends Component
{
    ...
    {
        $subcategory = TemplateSubCategory::where('slug', $this->subcategory_slug)->first();
        $latests = Template::where('status', 1)->latest()->take(5)->get();
        $subcats = TemplateSubCategory::where('status', 1)->get();

        return view('livewire.front-template-by-sub-category', [
